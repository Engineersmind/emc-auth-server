...
//
// See migration 00070 for why these are per-tenant data rather than constants.
// ---------------------------------------------------------------------------
...
type LockoutPolicy struct {
	...
	// migration 00070 for why that is available but not the default.
...
// DefaultLockoutPolicy mirrors the platform-default row seeded by migration 00070.
//
...
func (s *LockoutPolicyService) load(ctx context.Context, tenantID int64, applicationID *int64) (LockoutPolicy, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			// The platform-default row is seeded by migration 00070, so this
...
// a policy the operator did not choose. The CHECK constraints in migration 00070
...
func ApplyLockoutEnvOverrides(ctx context.Context, pool *pgxpool.Pool, ov LockoutEnvOverrides, logger zerolog.Logger) error {
	...
	if ov.HardLockDisabled {
		...
		// is the DoS-prone posture migration 00070 exists to move away from.
